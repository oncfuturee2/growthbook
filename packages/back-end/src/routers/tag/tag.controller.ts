...
import { addTag, removeTag, batchUpdateTagColors } from "back-end/src/models/TagModel";
...
type BatchUpdateTagColorRequest = AuthRequest<{ tags: string[]; color: string }>;

type BatchUpdateTagColorResponse = {
  status: 200;
};
...
export const putTagColor = async (
  req: BatchUpdateTagColorRequest,
  res: Response<BatchUpdateTagColorResponse | ApiErrorResponse>,
) => {
  const context = getContextFromReq(req);
  ...
  await batchUpdateTagColors(context.org.id, tags, color);
